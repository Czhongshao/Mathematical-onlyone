...
轮岗生产优化模型是针对工人每4周必须轮换工种的约束条件下，设计24周生产计划的数学模型。该模型旨在最大化总利润，同时满足工人技能匹配、工序需求和轮岗要求等约束条件。
...
将24周划分为6个4周周期，记为$k = 1, 2, \ldots, 6$。每个周期内工人分配保持不变，相邻周期之间工人需要更换工种。

#### 2. 决策变量

- $x_{ijk}$：第$k$个周期内第$i$级技工分配到第$j$道工序的人数，$i=1,2,\ldots,5$，$j=1,2,\ldots,5$，$k=1,2,\ldots,6$
- $y_{ik}$：第$k$个周期内第$i$级技工提升到$i+1$级的人数，$i=1,2,\ldots,4$，$k=1,2,\ldots,6$

#### 3. 约束条件

##### 3.1 技能匹配约束

若第$i$级技工不能从事第$j$道工序，则$x_{ijk} = 0$。根据技能矩阵：

$$x_{ijk} = 0, \quad \forall (i,j,k) \text{ 满足 } skill\_matrix[i,j] = 0$$

**符号说明**：

- $x_{ijk}$：第$k$个周期内第$i$级技工分配到第$j$道工序的人数
- $skill\_matrix[i,j]$：技能匹配矩阵，值为0表示第$i$级技工不能从事第$j$道工序

##### 3.2 人员总数约束

对于每个周期$k$和每个技工等级$i$：

$$\sum_{j=1}^{5} x_{ijk} + y_{ik} \leq N_{ik}, \quad \forall i=1,2,3,4, \forall k=1,2,\ldots,6$$

**符号说明**：

- $\sum_{j=1}^{5} x_{ijk}$：第$k$个周期内第$i$级技工分配到所有工序的总人数
- $y_{ik}$：第$k$个周期内第$i$级技工提升到$i+1$级的人数
- $N_{ik}$：第$k$个周期开始时第$i$级技工的总人数

对于最高级技工（第5级）：

$$\sum_{j=1}^{5} x_{5jk} \leq N_{5k} + t_{4,k}, \quad \forall k=1,2,\ldots,6$$

**符号说明**：

- $\sum_{j=1}^{5} x_{5jk}$：第$k$个周期内第5级技工分配到所有工序的总人数
- $N_{5k}$：第$k$个周期开始时第5级技工的总人数
- $t_{4,k}$：第$k$个周期内从第4级提升到第5级的技工人数

##### 3.3 工序需求约束

对于每个周期$k$和每个工序$j$：

$$\sum_{i=1}^{5} x_{ijk} = R_j, \quad \forall j=1,2,\ldots,5, \forall k=1,2,\ldots,6$$

**符号说明**：

- $\sum_{i=1}^{5} x_{ijk}$：第$k$个周期内所有级别技工分配到第$j$道工序的总人数
- $R_j$：第$j$道工序所需的工人总数

##### 3.4 轮岗约束

对于每个技工等级$i$、工序$j$和相邻周期$k$与$k+1$：

$$x_{ijk} + x_{ij,k+1} \leq N_{ik}, \quad \forall i=1,2,\ldots,5, \forall j=1,2,\ldots,5, \forall k=1,2,\ldots,5$$

**符号说明**：

- $x_{ijk}$：第$k$个周期内第$i$级技工分配到第$j$道工序的人数
- $x_{ij,k+1}$：第$k+1$个周期内第$i$级技工分配到第$j$道工序的人数
- $N_{ik}$：第$k$个周期开始时第$i$级技工的总人数
...
$$N_{i,k+1} = N_{ik} - y_{ik} + y_{i-1,k}, \quad \forall i=2,3,4,5, \forall k=1,2,\ldots,5$$

**符号说明**：

- $N_{i,k+1}$：第$k+1$个周期开始时第$i$级技工的总人数
- $N_{ik}$：第$k$个周期开始时第$i$级技工的总人数
- $y_{ik}$：第$k$个周期内第$i$级技工提升到$i+1$级的人数
- $y_{i-1,k}$：第$k$个周期内第$i-1$级技工提升到$i$级的人数

$$N_{1,k+1} = N_{1k} - y_{1k}, \quad \forall k=1,2,\ldots,5$$

**符号说明**：

- $N_{1,k+1}$：第$k+1$个周期开始时第1级技工的总人数
- $N_{1k}$：第$k$个周期开始时第1级技工的总人数
- $y_{1k}$：第$k$个周期内第1级技工提升到第2级的人数

#### 4. 目标函数

目标是最大化24周总利润，即6个周期利润之和：

$$\max P = \sum_{k=1}^{6} P_k$$

**符号说明**：

- $P$：24周总利润
- $P_k$：第$k$个周期的利润
- $\sum_{k=1}^{6} P_k$：6个周期利润之和

##### 4.1 产能计算

每个周期$k$中，工序$j$的产能为：

$$Q_{jk} = \sum_{i=1}^{5} x_{ijk} \cdot e_{ij}$$

**符号说明**：

- $Q_{jk}$：第$k$个周期内第$j$道工序的产能（件/天）
- $x_{ijk}$：第$k$个周期内第$i$级技工分配到第$j$道工序的人数
- $e_{ij}$：第$i$级技工在第$j$道工序的工作效率（件/天/人）
...
- $Q_k$：第$k$个周期的瓶颈产能（件/天）
- $\min_{j=1,\ldots,5} Q_{jk}$：所有工序产能中的最小值

##### 4.2 故障损失计算

周期$k$中，工序$j$的平均故障率：

$$F_{jk} = \frac{\sum_{i=1}^{5} x_{ijk} \cdot f_{ij}}{\sum_{i=1}^{5} x_{ijk}}$$

**符号说明**：

- $F_{jk}$：第$k$个周期内第$j$道工序的平均故障率
- $x_{ijk}$：第$k$个周期内第$i$级技工分配到第$j$道工序的人数
- $f_{ij}$：第$i$级技工在第$j$道工序的故障率
- $\sum_{i=1}^{5} x_{ijk} \cdot f_{ij}$：加权故障率总和
- $\sum_{i=1}^{5} x_{ijk}$：分配到第$j$道工序的总人数

周期$k$中，工序$j$的故障损失：

$$D_{jk} = \sum_{i=1}^{5} x_{ijk} \cdot f_{ij} \cdot (H \cdot S_k - T_{repair,j}) \cdot L_j$$

**符号说明**：

- $D_{jk}$：第$k$个周期内第$j$道工序的故障损失（元）
- $x_{ijk}$：第$k$个周期内第$i$级技工分配到第$j$道工序的人数
- $f_{ij}$：第$i$级技工在第$j$道工序的故障率（次/小时）
- $H$：每天工作小时数（小时/天）
- $S_k$：第$k$个周期的工作天数（天）
- $T_{repair,j}$：第$j$道工序单次故障修复时间（小时）
- $L_j$：第$j$道工序单次故障损失（元/次）
...
- $C_k$：第$k$个周期的总培训费用（元）
- $y_{ik}$：第$k$个周期内第$i$级技工提升到$i+1$级的人数
- $c_i$：第$i$级技工提升到$i+1$级的单位培训费用（元/人）
- $\sum_{i=1}^{4} y_{ik} \cdot c_i$：所有级别技工培训费用之和

##### 4.4 周期利润计算

周期$k$的总产量：

$$TP_k = Q_k \cdot S_k \cdot \text{num\_lines}$$
